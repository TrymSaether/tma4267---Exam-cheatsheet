...
{\color{rem-color}\normalfont\normalsize\bfseries}
{\color{rem-color}\thesection}{1em}{}


% Style for important formulas
...
\begin{multicols}{3}
        ...
        \begin{tabular}{r|cc}
                \scriptsize                 & \scriptsize\textbf{Order} & \scriptsize\textbf{No Order} \\ \hline
                \scriptsize\textbf{With R.} & $ n^k$                    & $\binom{n+k-1}{k}$           \\
                \scriptsize\textbf{No R.}   & $\frac{n!}{(n-k)!}$       & $\binom{n}{k}$
        \end{tabular}
        \section{Thinking Conditionally}
        ...
        Bayes' Rule is a special case of the LOTP. It allows you to find the probability of an event given that another event has occurred. For example, if you want to know the probability of $A$ given $B$, you can use Bayes' Rule to find it.
        \begin{align*}
        ...
        \section{Expected Value and Indicators}
        ...
        \section{Continuous RVs, LOTUS,  UoU}
        ...
        \section{Covariance and Transformations}
        ...
        If $X_{1},\dots,X_{n}$ are i.i.d. with \textbf{CDF} $F(x)$, then
        \begin{align*}
                F_{X_{\min}}(x) & = 1-[1-F(x)]^{\,n} \\
                F_{X_{\max}}(x) & = [F(x)]^{\,n}
        \end{align*}

        For a random vector $X\in\mathbb{R}^p$:
        \begin{align*}
                \mathrm{Cov}(X)        & = E[(X-E[X])(X-E[X])^T]                                                     \\
                \mathrm{Var}(a^T X)    & = a^T\mathrm{Cov}(X)a                                                       \\
                \mathrm{Corr}(X_i,X_j) & = \frac{\mathrm{Cov}(X_i,X_j)}{\sqrt{\mathrm{Var}(X_i)\,\mathrm{Var}(X_j)}}
        \end{align*}
        If $a$ is a constant vector, $\mathrm{Var}(a^T X)=a^T\,\mathrm{Cov}(X)\,a.$
        ...
        \keyformula{$f_X(x) = \frac{1}{(2\pi)^{p/2}|\Sigma|^{1/2}}\exp\{-\frac{1}{2}(x-\mu)^T\Sigma^{-1}(x-\mu)\}$}

        \textit{Properties:}
        \begin{itemize}[leftmargin=*,topsep=0pt,itemsep=0pt]
                \item Linear combinations: $AX \sim N_k(A\mu,\,A\Sigma A^T)$
                \item Marginals: $X_I \sim N_{|I|}(\mu_I,\Sigma_{II})$
                \item For $X=\begin{pmatrix}X_1\\X_2\end{pmatrix}\sim N\big(\begin{pmatrix}\mu_1\\ \mu_2\end{pmatrix},\,\begin{pmatrix}\Sigma_{11}&\Sigma_{12}\\\Sigma_{21}&\Sigma_{22}\end{pmatrix}\big)$:
        \end{itemize}
        ...
        \begin{center}
                \begin{tikzpicture}[>=stealth,scale=0.9]
                        \draw[->] (0,0) -- (2.3,0) node[right] {{\scriptsize Col$(X)$}};
                        \draw[->] (0,0) -- (0,1.4) node[above] {{\scriptsize Residual}};
                        \coordinate (Y) at (1, 1);
                        \draw[->, thick] (0,0) -- (Y) node[above right] {{\scriptsize $Y$}};
                        \draw[dashed] (Y) -- (1,0);
                        \draw[->, thick] (0,0) -- (1,0) node[below] {{\scriptsize $\hat Y$}};
        ...
        \textit{Properties:}
        \begin{itemize}[leftmargin=*,topsep=0pt,itemsep=0pt]
                \item $E[\hat{\beta}] = \beta$ (unbiased)
                \item $\mathrm{Var}(\hat{\beta})=\sigma^2 (X^T X)^{-1}$
                \item Gauss--Markov: $\hat{\beta}$ is BLUE (best linear unbiased)
                \item $\hat{\beta} \sim N(\beta,\;\sigma^2(X^T X)^{-1})$
                \item $\hat{Y} \sim N(X\beta,\;\sigma^2 H)$
                \item $e \sim N(0,\;\sigma^2 (I-H))$
                \item $\hat{Y}$ and $e$ are independent (since $H(I-H)=0$)
        \end{itemize}

        \keyformula{With intercept: $\mathrm{SST}=\mathrm{SSR} + \mathrm{SSE}$\\
                where $\mathrm{SST}=\sum(Y_i - \bar Y)^2$, $\mathrm{SSR}=\sum(\hat{Y}_i - \bar Y)^2$, $\mathrm{SSE}=\sum(Y_i-\hat{Y}_i)^2$}

        Degrees of freedom: $\mathrm{df}_{SSR}=p-1$, $\mathrm{df}_{SSE}=n-p$, $\mathrm{df}_{SST}=n-1$

        $\mathrm{SSE}/\sigma^2 \sim \chi^2_{\,n-p}$ and independent of $\hat{\beta}$

        Unbiased variance estimator: $s^2 = \mathrm{SSE}/(n-p)$

        \begin{center}
                \begin{tikzpicture}
                        \begin{axis}[width=4.5cm, height=3.5cm, xlabel={$x$}, ylabel={$y$},
                                        xlabel style={font=\scriptsize}, ylabel style={font=\scriptsize},
                                        tick label style={font=\scriptsize},
                                        xmin=-0.3, xmax=3.3, ymin=0.5, ymax=5.5,
                                        axis lines=left, xtick={0,1,2,3}, ytick={1,2,3,4,5}]
                                \addplot[domain=0:3, color=blue, thick] {1.2 + 1.2*x};
                                \addplot[only marks, mark=*, mark size=1.5pt] coordinates {(0,1) (1,3) (2,3) (3,5)};
                                \addplot[dashed, color=red] coordinates {(0,1) (0,1.2)};
                                \addplot[dashed, color=red] coordinates {(1,2.4) (1,3)};
                                \addplot[dashed, color=red] coordinates {(2,3) (2,3.6)};
                                \addplot[dashed, color=red] coordinates {(3,4.8) (3,5)};
                        \end{axis}
                \end{tikzpicture}
        \end{center}

        \section{Inference in Linear Model}
        For parameter $\beta_j$, under $H_0:\beta_j=0$:
        \keyformula{$t = \frac{\hat{\beta}_j}{s\,\sqrt{(X^T X)^{-1}_{jj}}} \sim t_{\,n-p}$}

        $100(1-\alpha)\%$ CI for $\beta_j$:
        $\hat{\beta}_j \pm t_{\,n-p,\;\alpha/2}\; s\,\sqrt{(X^T X)^{-1}_{jj}}$

        For prediction at $x_0$:
        \begin{itemize}[leftmargin=*,topsep=0pt,itemsep=0pt]
                \item $\hat{y}_0 = x_0 \hat{\beta}$
                \item $\mathrm{Var}(\hat{y}_0) = \sigma^2\,x_0 (X^T X)^{-1} x_0^T$
                \item CI for mean: $\hat{y}_0 \pm t_{\,n-p,\;\alpha/2}\; s\,\sqrt{x_0 (X^T X)^{-1} x_0^T}$
                \item PI for new $Y$: $\hat{y}_0 \pm t_{\,n-p,\;\alpha/2}\; s\,\sqrt{x_0 (X^T X)^{-1} x_0^T + 1}$
        \end{itemize}

        \textcolor{formulacolor}{Overall significance} (model with intercept):
        \keyformula{$F = \frac{\mathrm{SSR}/(p-1)}{\mathrm{SSE}/(n-p)} \sim F_{\,p-1,\;n-p}$}

        Testing subset of $d$ coefficients:
        \keyformula{$F = \frac{(\mathrm{SSE}_{\text{red}} - \mathrm{SSE}_{\text{full}})/d}{\mathrm{SSE}_{\text{full}}/(n-p)} \sim F_{\,d,\,n-p}$}

        Model fit measures:
        \begin{itemize}[leftmargin=*,topsep=0pt,itemsep=0pt]
                \item $R^2 = \mathrm{SSR}/\mathrm{SST}$ (proportion explained)
                \item Adjusted $R^2 = 1 - \frac{\mathrm{SSE}/(n-p)}{\mathrm{SST}/(n-1)}$
        \end{itemize}

        \section{Model Selection}
        Selection criteria (smaller is better):
        \begin{align*}
                \text{AIC} & = n\ln(\mathrm{SSE}/n) + 2p     \\
                \text{BIC} & = n\ln(\mathrm{SSE}/n) + p\ln n
        \end{align*}
